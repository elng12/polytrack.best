...
- 站内统一规则是：除根首页 `/` 外，最终 URL 都不带尾斜杠。
- 根首页最终 URL 是 `/`。
- 所有其他页面最终 URL 都不带尾斜杠，包括 `/blog`、`/es`、`/es/blog`、`/pt-br`、`/pt-br/blog`。
- `/es/` 必须 301 到 `/es`。
- `/es/blog/` 必须 301 到 `/es/blog`。
- 如果当前托管平台暂时无法做到 301，也必须确保 canonical、sitemap、hreflang、og:url、内链只使用 `/es`、`/es/blog`，并在后续补 301。
- canonical、sitemap、hreflang、og:url 必须全部使用最终 URL。
- 如果线上某个地址会 301 到另一个地址，PRD 和生成脚本必须使用 301 后的最终地址。
...
- cache manifest 不能包含 `/es/`、`/es/blog/` 这类非最终 URL。
...
- sitemap URL 不能出现双斜杠。
- sitemap 不能收录 `/es/`、`/es/blog/` 这类非最终 URL。
...
- `/en/blog/x` 必须最终到 `/blog/x`，不能变成 `/blog/x.html` 或 `/blog/x/`。
...
- 访问 `/es/` 必须 301 到 `/es`。
- 访问 `/es/blog/` 必须 301 到 `/es/blog`。
- 如果 P0-A 暂时无法做到 301，必须确认 canonical、sitemap、hreflang、og:url、内链只使用无尾斜杠最终 URL，并把补 301 列为 P0-B 前置任务。
- sitemap 只能收录最终版本，不收录带尾斜杠版本。
...
- 语言切换器没有 404。
- `/es/`、`/es/blog/` 已 301 到无尾斜杠最终 URL；如果暂时没做到，补 301 必须列为 P0-B 前置任务。
